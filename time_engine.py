...
# (1) Yield-based ODIM clock
def clock_metric_yield(X):
    return np.diag([
        0.0,
        0.0,
        20.0 / (X["sigma"]**2 + 1e-12),
        0.0,
        0.0,
        10.0 / (X["collapse_metric"]**2 + 1e-12)
    ])
...
df["theta_yield"] = compute_clock_time(df, clock_metric_yield)

# (2) Experimental-style clock: integrated yield (σ) along the path
theta_exp = [0.0]
for i in range(len(df)-1):
    sigma_mid = 0.5 * (df.iloc[i]["sigma"] + df.iloc[i+1]["sigma"])
    theta_exp.append(theta_exp[-1] + sigma_mid)
df["theta_exp"] = theta_exp
...
df["theta_yield_norm"] = normalize_series(df["theta_yield"])
df["theta_exp_norm"] = normalize_series(df["theta_exp"])
...
geo_path_disc, geo_length_disc = dijkstra(graph_O2, start=0, end=len(df)-1)
print("Discrete geodesic path indices (O2 metric):", geo_path_disc)
print("Discrete geodesic length (O2 metric):", geo_length_disc)

# ============================================================
# CONTINUOUS GEODESIC LENGTH (O2 METRIC, LINE INTERPOLATION)
# ============================================================

def continuous_geodesic_length(df, i, j, metric_fn, n_samples=200):
    X = df[["x","Q2","sigma","curvature","projection_capacity","collapse_metric"]].values
    Xi = X[i]
    Xj = X[j]
    length = 0.0
    for k in range(n_samples):
        ...
        dX = X2 - X1
        Xm = 0.5 * (X1 + X2)
        dists = np.linalg.norm(X - Xm, axis=1)
        idx = int(np.argmin(dists))
        g = metric_fn(df.iloc[idx])
        ds = math.sqrt(dX.T @ g @ dX)
        length += ds
    return length

geo_length_cont = continuous_geodesic_length(df, 0, len(df)-1, fisher_metric_O2, n_samples=500)
print("Continuous geodesic length (straight line, O2 metric):", geo_length_cont)

# ============================================================
# OBSERVER TIME DIVERGENCE (NEW)
# ============================================================

df["dt_O2_O1"] = df["t_O2"] - df["t_O1"]
df["dt_O3_O1"] = df["t_O3"] - df["t_O1"]
df["dt_O3_O2"] = df["t_O3"] - df["t_O2"]
...
corr_cols = [
    "t_O1","t_O2","t_O3",
    "theta_yield","theta_exp",
    "kappa_O1","kappa_O2","kappa_O3",
    "dt_O2_O1","dt_O3_O1","dt_O3_O2",
    "curvature","projection_capacity","collapse_metric","sigma","y_center"
]
corr_cols = [c for c in corr_cols if c in df.columns]
corr = df[corr_cols].corr()
corr.to_csv("time_engine_correlations.csv")
print("Saved time_engine_correlations.csv")
...
# Yield-based clock vs observers (raw)
plt.figure(figsize=(7,5))
plt.plot(df["theta_yield"], label="θ_yield", color="black", linestyle="--")
...
plt.title("Yield-based ODIM clock vs observer times")
plt.legend()
plt.tight_layout()
plt.savefig("clock_yield_vs_observers.png", dpi=300)
plt.close()

# Experimental clock vs observers (raw)
plt.figure(figsize=(7,5))
plt.plot(df["theta_exp"], label="θ_exp (integrated σ)", color="red", linestyle="--")
plt.plot(df["t_O1"], label="t_O1", alpha=0.7)
plt.plot(df["t_O2"], label="t_O2", alpha=0.7)
plt.plot(df["t_O3"], label="t_O3", alpha=0.7)
plt.xlabel("Step index")
plt.ylabel("Time parameter")
plt.title("Experimental-style clock vs observer times")
plt.legend()
plt.tight_layout()
plt.savefig("clock_exp_vs_observers.png", dpi=300)
plt.close()

# Normalized clocks vs normalized observers
plt.figure(figsize=(7,5))
plt.plot(df["theta_yield_norm"], label="θ_yield (norm)", color="black", linestyle="--")
plt.plot(df["theta_exp_norm"], label="θ_exp (norm)", color="red", linestyle="--")
plt.plot(df["t_O1_norm"], label="O1 (norm)")
plt.plot(df["t_O2_norm"], label="O2 (norm)")
plt.plot(df["t_O3_norm"], label="O3 (norm)")
plt.xlabel("Step index")
plt.ylabel("Normalized time")
plt.title("Normalized clocks vs normalized observer times")
plt.legend()
plt.tight_layout()
plt.savefig("clocks_vs_observers_normalized.png", dpi=300)
plt.close()
...
for i, row in df.iterrows():
    plt.text(row["mds_x"], row["mds_y"], str(i), fontsize=6)
...
# NEW: time divergence vs step
plt.figure(figsize=(7,5))
plt.plot(df["dt_O2_O1"], label="Δt(O2 - O1)")
plt.plot(df["dt_O3_O1"], label="Δt(O3 - O1)")
plt.plot(df["dt_O3_O2"], label="Δt(O3 - O2)")
plt.xlabel("Step index")
plt.ylabel("Time difference")
plt.title("Observer time divergence (pairwise)")
plt.legend()
plt.tight_layout()
plt.savefig("observer_time_divergence_pairwise.png", dpi=300)
plt.close()

# NEW: time divergence vs rapidity
plt.figure(figsize=(7,5))
plt.scatter(df["y_center"], df["dt_O2_O1"], label="Δt(O2 - O1)", alpha=0.7)
plt.scatter(df["y_center"], df["dt_O3_O1"], label="Δt(O3 - O1)", alpha=0.7)
plt.scatter(df["y_center"], df["dt_O3_O2"], label="Δt(O3 - O2)", alpha=0.7)
plt.xlabel("y_center")
plt.ylabel("Time difference")
plt.title("Observer time divergence vs rapidity")
plt.legend()
plt.tight_layout()
plt.savefig("observer_time_divergence_vs_y.png", dpi=300)
plt.close()

...
def fisher_metric_O3(X):
    # O3: cares about sigma + collapse_metric (like a "yield-sensitive" observer)
    return np.diag([
        0.1,
        0.1,
...
def clock_metric(X):
    ...
    # Treats changes in projection_capacity as its "ticks"
    return np.diag([
        0.0,  # x
        0.0,  # Q2
        0.0,  # sigma
        0.0,  # curvature
        50.0 / (X["projection_capacity"]**2 + 1e-12),  # main clock axis
        0.0   # collapse_metric
    ])
...
# BASIC "GEODESIC" VIEW
# ============================================================
# Here, your data trajectory is treated as a discrete geodesic:
# t_O1, t_O2, t_O3, and theta_clock are all proper-length parameters
# along the same underlying path, in different metrics.

# You can now compare how each observer and the clock parameterize

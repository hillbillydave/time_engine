...
# (1) Yield-based ODIM clock (as before)
...
# Think: θ_exp ~ ∑ σ_k Δs  (here Δs = 1 per step, so just cumulative σ)
theta_exp = [0.0]
for i in range(len(df)-1):
...
def continuous_geodesic_length(df, i, j, metric_fn, n_samples=200):
    ...
    for k in range(n_samples):
        ...
        Xm = 0.5 * (X1 + X2)
        # approximate metric by nearest real point's metric
        # (closest index in original df)

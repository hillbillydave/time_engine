...
# ============================================================
# CLOCK SUBMANIFOLD (YIELD-BASED CLOCK)
# ============================================================

def clock_metric(X):
    # Clock tracks physical yield: sigma + collapse_metric
    return np.diag([
        0.0,  # x
        0.0,  # Q2
        20.0 / (X["sigma"]**2 + 1e-12),          # main axis: cross-section
        0.0,  # curvature
        0.0,  # projection_capacity
        10.0 / (X["collapse_metric"]**2 + 1e-12) # secondary: ODIM-compressed yield
    ])
...
df["theta_clock_norm"] = normalize_series(df["theta_clock"])
...
plt.title("Yield-based clock vs observer times")
plt.legend()
plt.tight_layout()
plt.savefig("clock_vs_observers_yield.png", dpi=300)
plt.close()

# Normalized clock vs normalized observers
plt.figure(figsize=(7,5))
plt.plot(df["theta_clock_norm"], label="Clock θ (norm)", color="black", linestyle="--")
plt.plot(df["t_O1_norm"], label="O1 (norm)")
plt.plot(df["t_O2_norm"], label="O2 (norm)")
plt.plot(df["t_O3_norm"], label="O3 (norm)")
plt.xlabel("Step index")
plt.ylabel("Normalized time")
plt.title("Normalized yield-based clock vs normalized observer times")
plt.legend()
plt.tight_layout()
plt.savefig("clock_vs_observers_normalized_yield.png", dpi=300)
plt.close()
